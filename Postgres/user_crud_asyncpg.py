...
async def create_user(conn:asyncpg.Connection,user_data:create_schema) -> response_schema:
    ...
    try:
        row = await conn.fetchrow("""
            ...
            queries_today, cooldown_until, last_query)
...
async def update_user(conn: asyncpg.Connection, user_id: int, update_data: upadate_schema) -> Optional[response_schema]:
    ...
    if not sets:
        return await get_by_id(conn, user_id) # if there is nothing to update just returning user :)
...
async def modify_user(user_id:int, update:upadate_schema,conn:asyncpg.Connection=Depends(get_conn)):
    updated=await update_user(conn,user_id, update)
    if not updated:
        raise HTTPException(404, "User not found")
    return updated
