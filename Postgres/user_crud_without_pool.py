...
class create_schema(BaseModel):
    ...
    short_term_memory: Optional[list] = None   # any JSON list
...
class upadate_schema(BaseModel):
    ...
    short_term_memory: Optional[list] = None
    summary: Optional[str] = None
...
async def create_user(conn: asyncpg.Connection, user_data: create_schema) -> response_schema:
    ...
    """, user_data.name, user_data.email, user_data.adv_rank, user_data.mains_rank,
       user_data.category.value, user_data.gender.value, preferred_branches_json,
       usage.queries_today, usage.cooldown_until, usage.last_query,
       short_term_memory_json, user_data.summary)
    if row is None:
    ...
    return response_schema(
        ...
        adv_rank=row["adv_rank"], mains_rank=row["mains_rank"],
        category=Category(row["category"]), gender=Gender(row["gender"]),
        preferred_branches=preferred_branches_list, updated_at=row["updated_at"],
        usage=usage_schema(queries_today=row["queries_today"], cooldown_until=row["cooldown_until"],
                           last_query=row["last_query"]),
        short_term_memory=short_term_memory_list, summary=row["summary"]
    )
...
async def update_user(conn: asyncpg.Connection, email: str, update_data: upadate_schema) -> Optional[response_schema]:
    ...
    if update_data.name is not None:
        sets.append(f"name = ${idx}"); params.append(update_data.name); idx += 1
    if update_data.adv_rank is not None:
        sets.append(f"adv_rank = ${idx}"); params.append(update_data.adv_rank); idx += 1
    if update_data.mains_rank is not None:
        sets.append(f"mains_rank = ${idx}"); params.append(update_data.mains_rank); idx += 1
    if update_data.category is not None:
        sets.append(f"category = ${idx}"); params.append(update_data.category.value); idx += 1
    if update_data.gender is not None:
        sets.append(f"gender = ${idx}"); params.append(update_data.gender.value); idx += 1
    if update_data.preferred_branches is not None:
        sets.append(f"preferred_branches = ${idx}"); params.append(json.dumps(update_data.preferred_branches)); idx += 1
    if update_data.usage is not None:
        sets.append(f"queries_today = ${idx}"); params.append(update_data.usage.queries_today); idx += 1
        sets.append(f"cooldown_until = ${idx}"); params.append(update_data.usage.cooldown_until); idx += 1
        sets.append(f"last_query = ${idx}"); params.append(update_data.usage.last_query); idx += 1
    if update_data.short_term_memory is not None:
        sets.append(f"short_term_memory = ${idx}::jsonb"); params.append(json.dumps(update_data.short_term_memory)); idx += 1
    if update_data.summary is not None:
        sets.append(f"summary = ${idx}"); params.append(update_data.summary); idx += 1
...
# --- FastAPI app (no lifespan needed for pooling; connections handled per request) ---
app = FastAPI()
...
@app.patch("/users/by-email/{email}", response_model=response_schema)   # changed path to accept email
...
# (Optional) Keep get_pool if needed elsewhere, but not used anymore.
def get_pool():
    return None
