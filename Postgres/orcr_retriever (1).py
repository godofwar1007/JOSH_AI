...
class ORCR_Retriever:
    async def runa(self,ja_rank,category,gender):#for advanced rank
        conn = await asyncpg.connect(user='postgres', password='postgres',
                                    database='orcr_data', host='localhost',port=5555)
...
async def main():
    ...
    options=await retriever.runa(18500,"OPEN","Gender-Neutral")
    options=await retriever.runm(50000,"OPEN","Gender-Neutral")
    print(options)
