...
class ORCR_Retriever:
    async def run(self,rank,category,gender):
        ...
        for row in values:
            ...
            record={"Institute":institute,"Academic program":row["academic_program"],"Opening Rank":r,"Closing Rank":cr,"Seat Type":row["rank_type"]}
...
def main():
    ...
    options=asyncio.run(retriever.run(1688,"OPEN","Gender-Neutral"))
    print(options)
